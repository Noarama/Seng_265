 ...
 *
 */
node_t *new_node(char *val , int statistic)
{
    assert(val != NULL);
...
/**
 * Function:  add_inorder_stat
 * ----------------------
 * @brief  Allows to add a new node to the list respecting the order of its statistic.
 *
 * @param list The list where the node will be added (i.e., a pointer to the first element in the list).
 * @param new The node to be added to the list.
 *
 ...
 *
 */
node_t *add_inorder_stat(node_t *list, node_t *new)
{
    node_t *prev = NULL;
    node_t *curr = NULL;
    ...
    for (curr = list; curr != NULL; curr = curr->next)
    {
        ...
        if (new->stat < curr->stat)
            prev = curr;

        else if (new->stat == curr->stat){
            if (strcmp(new->word, curr->word) > 0)
            prev = curr;
        }
        else
